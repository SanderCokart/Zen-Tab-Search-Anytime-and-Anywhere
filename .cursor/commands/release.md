Prepare and publish a new release of the CCV Shop Firefox extension. Run commands yourself unless blocked (e.g. missing AMO credentials).
...
This runs build, web-ext lint, and submits an unlisted signing request. If credentials are missing, stop and explain the setup from `README.md` (encrypt with `npm run env:encrypt:local`).
...
- On success, the signed file lands in `signed/ccvshop-dev-helper-<version>.xpi`.
...
2. Commit with a clear message, e.g. `chore(release): v1.4.0`
